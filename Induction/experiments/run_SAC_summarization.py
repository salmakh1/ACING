...
def run(task, n_prompt_tokens, HF_cache_dir,  total_iter, random_proj,
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f"SAC_summarization_task_{task}_seed_{args.seed}.log"),
    ...
    # best_prompt, best_r = acb.run_ACB()

    bounds = np.array([[0, 1]] * intrinsic_dim)
    agent = SACAgent(dim=bounds.shape[0], llm=model_forward_api)

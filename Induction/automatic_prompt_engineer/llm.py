...
from LlamaForMLPRegression import LlamaForMLPRegression
...
def model_from_config(config, disable_tqdm=True):
    """Returns a model based on the configs."""
...
class Llama_Forward(LLM):
    ...
    def __init__(self, config, needs_confirmation=False, disable_tqdm=True):
        ...
        MODEL_DIR = './models--lmsys--vicuna-13b-v1.3/snapshots/6566e9cb1787585d1147dcf4f9bc48f29e1328d2'
        # TOKENIZER_DIR = '/data/bobchen/llama4trans/tokenizer'
        self.config = config
        ...
        self.model = LlamaForMLPRegression.from_pretrained(MODEL_DIR, low_cpu_mem_usage=True, **kwargs).to(self.device)
    ...
    def generate_text(self, prompts, n):
        print("prompts inside generate text", len(prompts))
        ...
        for prompt in prompts:
            ...
            discrete_prompt = self.tokenizer.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[
                    0]
            # print(discrete_prompt)
            discrete_prompt = discrete_prompt.replace('[APE]', '').strip()

            # for i in range(len(discrete_prompt)):
            #     discrete_prompt[i] = discrete_prompt[i].replace('[APE]', '').strip()
            text.append(discrete_prompt)
...
class Flan_T5(LLM):
    ...
    def generate_text(self, prompts, n):
        ...
        # if not self.disable_tqdm:
        #     print(
        #         f"[{self.configs['name']}] Generating {len(prompt) * n} completions, "
        #         f"split into {len(prompt_batches)} batches of size {batch_size * n}")
        text = []
        batch_size = 10
